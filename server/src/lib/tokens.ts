...
export const signAccess = (u: TokenUser) =>
  jwt.sign({ id: u.id, role: u.role, email: u.email }, accessSecret, { expiresIn: '15m' });
export const signRefresh = (u: TokenUser) =>
  jwt.sign({ id: u.id, role: u.role, email: u.email }, refreshSecret, { expiresIn: '7d' });
